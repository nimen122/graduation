...
public class SingleChartServiceImpl implements SingleChartService {
    ...
    public Result IMR(SingleChartParam singleChartParam) {
        ...
        //计算单值控制图的CL
        double cl_SingleValue = 0.0;
        if (singleChartParam.getHistoryAve() != -1){
            cl_SingleValue = singleChartParam.getHistoryAve();
        }else {
            cl_SingleValue = Calculate.CLOfXbar(singleData);
        }
        //计算sigma
        double sigma = 0.0;
        if (singleChartParam.getHistorySigma() != -1){
            sigma = singleChartParam.getHistorySigma();
        }else {
    ...
    public Result I(SingleChartParam singleChartParam) {
        ...
        if (singleChartParam.getHistorySigma() !=-1){
            ...
            switch (sigmaMode) {
                case "movingRangeAverageAsSigma":
                    sigma = Calculate.movingRangeAverageAsSigma(groupMovingRange, movingRangeLength);
                    break;
                case "movingRangeMedianAsSigma":
                    sigma = Calculate.movingRangeMedianAsSigma(groupMovingRange, movingRangeLength);
                    break;
                case "SRMSSDAsSigmaWithConstant":
                    sigma = Calculate.SRMSSDAsSigmaWithConstant(groupMovingRange, singleData.size());
                    break;
                case "SRMSSDAsSigmaWithoutConstant":
    ...
    @Override
    public Result delI(DelSingleChartParam singleChartParam) {
        List<Double> singleData = singleChartParam.getSourceData();
        //去除要删的组
        Collections.sort(singleChartParam.getDelPoint());
        for (int i = singleChartParam.getDelPoint().size()-1;i>=0;i--){
            if (i <= singleData.size()){
                singleData.remove((int)singleChartParam.getDelPoint().get(i));
            }
        }
        int movingRangeLength = singleChartParam.getMovingRangeLength();
        String sigmaMode = singleChartParam.getSigmaMode();
    ...
    @Override
    public Result delMR(DelSingleChartParam singleChartParam) {
        List<Double> singleData = singleChartParam.getSourceData();
        //去除要删的组
        Collections.sort(singleChartParam.getDelPoint());
        for (int i = singleChartParam.getDelPoint().size()-1;i>=0;i--){
            if (i <= singleData.size()){
                singleData.remove(singleChartParam.getDelPoint().get(i) - 1);
            }
        }
        int movingRangeLength = singleChartParam.getMovingRangeLength();
        String sigmaMode = singleChartParam.getSigmaMode();
    ...
    @Override
    public Result delIMR(DelSingleChartParam singleChartParam) {
        List<Double> singleData = singleChartParam.getSourceData();
