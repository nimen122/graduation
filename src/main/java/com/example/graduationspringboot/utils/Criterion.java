...
public class Criterion {
    ...
    检验一：距离中心线大于 K 个标准差
     */
    public static ResultOfCriterion Criterion_1(List<Double> data,List<Double> lcl,List<Double> ucl,int k){
        ...
        double num = Math.sqrt(data.size());
...
//    }
    ...
    public static ResultOfCriterion Criterion_5(List<Double> data,List<Double> cl,Double standardDeviation,int k){
        ...
        for (int i = 0;i<data.size();i++){
            if (data.get(i)> cl.get(i)){
                upList.add(data.get(i));
                downList.add(cl.get(i));
            }else{
                downList.add(data.get(i));
                upList.add(cl.get(i));
            }
        }
        while(step <= upList.size()){
            int num = 0;
            for (int i = 0;i<k+1;i++){
                boolean current = false;
                if (upList.get(i + step - k - 1) > cl.get(i + step - k - 1)+2*standardDeviation){
                    num++;
                    current = true;
                }
                if (num>=k && current){
                    result.setFit(true);
                    errorPointIndex.add(i + step - k);
                }
            }
            step++;
        }
        step = k+1;
        while(step <= downList.size()){
            int num = 0;
            for (int i = 0;i<k+1;i++){
                boolean current = false;
                if (downList.get(i + step - k - 1) < cl.get(i + step - k - 1)-2*standardDeviation){
                    num++;
                    current = true;
                }
                if (num>=k && current){
        ...
        if(result.isFit()){
            errorPointIndex = errorPointIndex.stream().distinct().collect(Collectors.toList());
            Collections.sort(errorPointIndex);
            result.setMsg(MessageFormat.format(msg,k+1,k,errorPointIndex));
            result.setErrorPointIndex(errorPointIndex);
        }
        return result;
...
//        for(int i = 0;i<data.size();i++){
//            List<Integer> sixUpAbnormalIndex = new ArrayList<>(); // 中心线上方累计异常数据下标
//            List<Integer> sixDownAbnormalIndex = new ArrayList<>(); // 中心线下方累计异常数据下标
//            // 从第K个值开始统计，往前遍历是否有K-1个值符合条件
//            if (i >= k - 1) {
//                for (int j = 0; j < k; j++) {
//                    if (data.get(i-j) > cl.get(i-j) + standardDeviation && data.get(i) > cl.get(i)+standardDeviation) {
//                        sixUpAbnormalIndex.add(i+1);
//                    }
//                    if (data.get(i-j) < cl.get(i-j) - standardDeviation && data.get(i) < cl.get(i)-standardDeviation) {
//                        sixDownAbnormalIndex.add(i+1);
//                    }
//                }
//                if (sixUpAbnormalIndex.size() > k-1) {
//                    result.setFit(true);
//                    errorPointIndex.addAll(sixUpAbnormalIndex);
//                }
//                if (sixDownAbnormalIndex.size() > k-1) {
//                    result.setFit(true);
//                    errorPointIndex.addAll(sixDownAbnormalIndex);
//                }
//            }
//        }
//        if(result.isFit()){
...
//    }
    ...
    public static ResultOfCriterion Criterion_6(List<Double> data,List<Double> cl,Double standardDeviation,int k){
        ...
        while(step <= upList.size()){
            ...
            for (int i = 0;i<k+1;i++){
                boolean current = false;
                if (upList.get(i + step - k - 1) > cl.get(i + step - k - 1)+standardDeviation){
                    num++;
                    current = true;
                }
                if (num>=k && current){
        ...
        while(step <= downList.size()){
            ...
            for (int i = 0;i<k+1;i++){
                boolean current = false;
                if (downList.get(i + step - k - 1) < cl.get(i + step - k - 1)-standardDeviation){
                    num++;
                    current = true;
                }
                if (num>=k && current){
...
//    }
    ...
    public static List<ResultOfCriterion> ResultOfCriterionOfXbar(List<Double> dataXbar,List<Double> CLXbar,List<Double> LCLXbar,
                                                                  List<Double> UCLXbar,double sigma,String rules,List<Integer> rulesKey){

        List<ResultOfCriterion> resultOfCriterionList =new ArrayList<>();
        if (rules.charAt(0) == '1'){
            ResultOfCriterion resultOfCriterion_1 = Criterion.Criterion_1(dataXbar,LCLXbar,UCLXbar,rulesKey.get(0));
            resultOfCriterionList.add(resultOfCriterion_1);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(1) == '1'){
            ResultOfCriterion resultOfCriterion_2 = Criterion.Criterion_2(dataXbar,CLXbar,rulesKey.get(1));
            resultOfCriterionList.add(resultOfCriterion_2);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(2) == '1'){
            ResultOfCriterion resultOfCriterion_3 = Criterion.Criterion_3(dataXbar,rulesKey.get(2));
            resultOfCriterionList.add(resultOfCriterion_3);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(3) == '1'){
            ResultOfCriterion resultOfCriterion_4 = Criterion.Criterion_4(dataXbar,rulesKey.get(3));
            resultOfCriterionList.add(resultOfCriterion_4);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(4) == '1'){
            ResultOfCriterion resultOfCriterion_5 = Criterion.Criterion_5(dataXbar,CLXbar,sigma,rulesKey.get(4));
            resultOfCriterionList.add(resultOfCriterion_5);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(5) == '1'){
            ResultOfCriterion resultOfCriterion_6 = Criterion.Criterion_6(dataXbar,CLXbar,sigma,rulesKey.get(5));
            resultOfCriterionList.add(resultOfCriterion_6);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(6) == '1'){
            ResultOfCriterion resultOfCriterion_7 = Criterion.Criterion_7(dataXbar,CLXbar,sigma,rulesKey.get(6));
            resultOfCriterionList.add(resultOfCriterion_7);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(7) == '1'){
            ResultOfCriterion resultOfCriterion_8 = Criterion.Criterion_8(dataXbar,CLXbar,sigma,rulesKey.get(7));
            resultOfCriterionList.add(resultOfCriterion_8);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        return resultOfCriterionList;
    ...
    public static List<ResultOfCriterion> ResultOfCriterionOfRAndS(List<Double> groupAve, List<Double> CL_R, List<Double> LCL_R,
                                                                   List<Double> UCL_R, String rules, List<Integer> rulesKey) {
        List<ResultOfCriterion> resultOfCriterionList =new ArrayList<>();
        if (rules.charAt(0) == '1'){
            ResultOfCriterion resultOfCriterion_1 = Criterion.Criterion_1(groupAve,LCL_R,UCL_R,rulesKey.get(0));
            resultOfCriterionList.add(resultOfCriterion_1);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(1) == '1'){
            ResultOfCriterion resultOfCriterion_2 = Criterion.Criterion_2(groupAve,CL_R,rulesKey.get(1));
            resultOfCriterionList.add(resultOfCriterion_2);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(2) == '1'){
            ResultOfCriterion resultOfCriterion_3 = Criterion.Criterion_3(groupAve,rulesKey.get(2));
            resultOfCriterionList.add(resultOfCriterion_3);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(3) == '1'){
            ResultOfCriterion resultOfCriterion_4 = Criterion.Criterion_4(groupAve,rulesKey.get(3));
            resultOfCriterionList.add(resultOfCriterion_4);
        }
        return resultOfCriterionList;
    }

    public static List<ResultOfCriterion> ResultOfCriterionOfI(List<Double> sourceData, List<Double> cl_i, List<Double> lcl_i, List<Double> ucl_i, double sigma, String rules, List<Integer> rulesKey) {
        List<ResultOfCriterion> resultOfCriterionList =new ArrayList<>();
        if (rules.charAt(0) == '1'){
            ResultOfCriterion resultOfCriterion_1 = Criterion.Criterion_1(sourceData,lcl_i,ucl_i,rulesKey.get(0));
            resultOfCriterionList.add(resultOfCriterion_1);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(1) == '1'){
            ResultOfCriterion resultOfCriterion_2 = Criterion.Criterion_2(sourceData,cl_i,rulesKey.get(1));
            resultOfCriterionList.add(resultOfCriterion_2);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(2) == '1'){
            ResultOfCriterion resultOfCriterion_3 = Criterion.Criterion_3(sourceData,rulesKey.get(2));
            resultOfCriterionList.add(resultOfCriterion_3);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(3) == '1'){
            ResultOfCriterion resultOfCriterion_4 = Criterion.Criterion_4(sourceData,rulesKey.get(3));
            resultOfCriterionList.add(resultOfCriterion_4);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(4) == '1'){
            ResultOfCriterion resultOfCriterion_5 = Criterion.Criterion_5(sourceData,cl_i,sigma,rulesKey.get(4));
            resultOfCriterionList.add(resultOfCriterion_5);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(5) == '1'){
            ResultOfCriterion resultOfCriterion_6 = Criterion.Criterion_6(sourceData,cl_i,sigma,rulesKey.get(5));
            resultOfCriterionList.add(resultOfCriterion_6);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(6) == '1'){
            ResultOfCriterion resultOfCriterion_7 = Criterion.Criterion_7(sourceData,cl_i,sigma,rulesKey.get(6));
            resultOfCriterionList.add(resultOfCriterion_7);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(7) == '1'){
            ResultOfCriterion resultOfCriterion_8 = Criterion.Criterion_8(sourceData,cl_i,sigma,rulesKey.get(7));
            resultOfCriterionList.add(resultOfCriterion_8);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        return resultOfCriterionList;
    ...
    public static List<ResultOfCriterion> ResultOfCriterionOfMR(List<Double> groupMovingRange, List<Double> cl_mr, List<Double> lcl_mr, List<Double> ucl_mr, String rules, List<Integer> rulesKey) {
        List<ResultOfCriterion> resultOfCriterionList =new ArrayList<>();
        if (rules.charAt(0) == '1'){
            ResultOfCriterion resultOfCriterion_1 = Criterion.Criterion_1_MR(groupMovingRange,lcl_mr,ucl_mr,rulesKey.get(0));
            resultOfCriterionList.add(resultOfCriterion_1);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(1) == '1'){
            ResultOfCriterion resultOfCriterion_2 = Criterion.Criterion_2_MR(groupMovingRange,cl_mr,rulesKey.get(1));
            resultOfCriterionList.add(resultOfCriterion_2);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(2) == '1'){
            ResultOfCriterion resultOfCriterion_3 = Criterion.Criterion_3_MR(groupMovingRange,rulesKey.get(2));
            resultOfCriterionList.add(resultOfCriterion_3);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        if (rules.charAt(3) == '1'){
            ResultOfCriterion resultOfCriterion_4 = Criterion.Criterion_4_MR(groupMovingRange,rulesKey.get(3));
            resultOfCriterionList.add(resultOfCriterion_4);
        }else resultOfCriterionList.add(new ResultOfCriterion());
        return resultOfCriterionList;
    }
